...
public class ComponenteMapa extends JComponent implements ObservadorSimuladorTrafico {
	...
	private void drawMap(Graphics g) {
		...
		for (Carretera e : this.mapa.getCarreteras()) {
			...
			for (Vehiculo d : e.getCars()) {
				if (ultimaLocalizacion != d.getCurLocationOnRoad()) { 
					ultimaLocalizacion = d.getCurLocationOnRoad();
					diam = radioVehiculo;
				} else {
					diam += radioVehiculo;
				}
				Color dotColor = d.getTiempoInfraccion() > 0.5 ? Color.RED : Color.MAGENTA;
				drawCircleOnALine(g, p1.cX, p1.cY, p2.cX, p2.cY, e.getLength(), d.getCurLocationOnRoad(), diam, dotColor,
						d.getId());
			}
		}
	}
